...
$(document).on('click','.editSend',function(){
	//ajax
	var ids = [];
	otable.rows('.selected').data().each(function(row){
		ids.push(row.DT_RowId.replace('row_',''));
	});
	$.ajax({
            type: "POST",
            url: "https://bmskarate.com/admin/students/edit_students",
            headers: {'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')},
            data: {
				stripe: $('.all_popup [name="pop_stripe"]').val(),
				rank_id: $('.all_popup [name="pop_rank_id"]').val(),
				program_id: $('.all_popup [name="pop_program_id"]').val(),
				start_date: $('.all_popup [name="pop_startdate"]').val(),
				expiry: $('.all_popup [name="pop_expiry"]').val(),
				pop_lastexam: $('.all_popup [name="pop_lastexam"]').val(),
				pop_active: $('.all_popup [name="pop_active"]:checked').val(),
				ids:ids
				},
...
$(document).on('click','.noteSend',function(){
	var ids = [];
	otable.rows('.selected').data().each(function(row){
		ids.push(row.DT_RowId.replace('row_',''));
	});
		$.ajax({
            type: "POST",
            url: "https://bmskarate.com/admin/students/add_note",
            headers: {'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')},
            data: {note: $('[name="mass_note"]').val(), ids:ids},
            success: function(response) {
				if(response){
				$('[name="note"]').val();
				location.reload();
...
$('.mydatepicker').datepicker({
 ...
 $('.mydatepicker1').datepicker({
	...
	onSelect : function(mydate) {
        ...
        var newDate = new Date(dateAr[1] + '-' + dateAr[0] + '-' + dateAr[2]);
        var exp = new Date(newDate.setMonth(newDate.getMonth()+3));
    	$('#expiry').datepicker('setDate', exp);
	}
});
...
// profit Loss
 ...
 $(document).ready(function(){
	 $(".overlay").hide();
	 // $(".page-loader-body").removeClass('page-loader-body');
 });

...
<style>
	...
	-->
	<body class="smart-style-1 page-loader-body">
		<!-- #HEADER -->
		<header id="header">
			<div id="logo-group">
			...
			<div class="login-info">
				<span> <!-- User image size is adjusted inside CSS, it should stay as it -->
					...
					<a href="javascript:void(0);" id="show-shortcut" data-action="toggleShortcut">
						@php
						$d_filetype = Controller::dojo_filetype(auth()->user()->id);
						@endphp

						<img src="
						@if(auth()->user()->role == 1)
							@if(!empty(auth()->user()->filetype))
							{{asset('/uploads/admin/'.auth()->user()->id.auth()->user()->filetype)}}
							@else
							{{asset('/app_images/default.jpg')}}
							@endif
						@endif
						@if(auth()->user()->role == 2)
						@if(!empty($d_filetype))
						{{asset('/uploads/dojos/'.Controller::user_dojo(auth()->user()->id).$d_filetype)}}
						@else
						{{asset('/app_images/default.jpg')}}
						@endif
						@else
	...
	margin-top:-40px !important;
		...
		<div class="page-footer">
			<div class="row">
				<div class="col-xs-12 col-sm-6">
					<span class="txt-color-white">Accounting Software</span>
				</div>
		...
		<div id="shortcut">
			<ul>
				...
				@if(auth()->user()->role == 1)
				<li>
					<a href="{{route('admin.profile')}}" class="jarvismetro-tile big-cubes selected bg-color-orangeDark"> <span class="iconbox"> <i class="fa fa-user fa-4x"></i> <span>Profile</span> </span> </a>
				</li>
				@endif
			</ul>
		</div>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/fullcalendar/3.9.0/fullcalendar.js"></script>
	 ...
	 e.preventDefault();
	     ...
	     $.ajax({
            ...
            data: {note: $('[name="note"]').val(), id:$('[name="id"]').val()},
            success: function(response) {
				if(response){
				$('[name="note"]').val();
				window.location.href="https://bmskarate.com/admin/students/show";
				}
            }
        });
